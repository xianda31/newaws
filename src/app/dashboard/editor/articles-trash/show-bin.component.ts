...
@Component({
  selector: 'app-show-bin',
  templateUrl: './show-bin.component.html',
  styleUrl: './show-bin.component.scss'
})
export class ShowBinComponent implements OnInit {
  ...
  onSelect(item: { id: string, headline: string, rank: number }): void {
    console.log('selected', item.id, item.headline, item.rank);
