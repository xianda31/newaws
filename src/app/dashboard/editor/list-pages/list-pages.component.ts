...
export class ListPagesComponent {
  ...
  ngOnInit(): void {
    ...
    this.pages$.subscribe((pages) => {
      this.root_menus = {};
      pages.forEach((page) => {
        this.root_menus[page.root_menu] = this.root_menus[page.root_menu] ? this.root_menus[page.root_menu] + 1 : 1;
      });
      console.log('root_menus', this.root_menus);
      // this.verifyPath(pages);     // correction des paths menu vs sous-root_menus
    });
  }
  ...
  // verification des paths menu vs sous-root_menus
  // verifyPath(pages: Page[]) {

  //   pages.forEach((page) => {
  //     const { articles, ...pageWithoutArticles } = page;
  //     const root = (this.root_menus[page.root_menu] === 1) ? '' : page.root_menu + '/';
  ...
  //   newPage.path = (this.root_menus[newPage.root_menu] === 1) ? newPage.label.toLowerCase().replace(/\s/g, '-') : (newPage.root_menu + '/' + newPage.label).toLowerCase().replace(/\s/g, '-');
